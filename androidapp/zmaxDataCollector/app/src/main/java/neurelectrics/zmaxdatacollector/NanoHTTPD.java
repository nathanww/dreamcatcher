...
public abstract class NanoHTTPD {
    ...
    private boolean addFnPrefix = true;   // Add generated temp value to output file
    public void setAddFilenamePrefix(boolean afp){
        addFnPrefix = afp;  // save setting
    }
    private String savedFilesFolder = "";
    public void setSaveFilesFolder(String sff){
        savedFilesFolder = sff;
    }
    /**
...
//        System.out.println("NH SoTo="+myServerSocket.getSoTimeout());  // = 0 <<<<<<<<<<<<<<<
        ...
        myThread = new Thread(new Runnable() {
            ...
            public void run() {
                do {
                    try {
                        System.out.println("NH start waiting soc="+myServerSocket
                                + " inet="+myServerSocket.getInetAddress()); //<<<<<<<<<<<<
...
//            System.out.println("cTF fn="+tempFile.getName()); //<<<<<<<<<
//            Thread.dumpStack();                               //<<<<<<<<<<<<<<
            return tempFile;
        }
        ...
        private void parseBody(Map<String, String> files) throws IOException, ResponseException {
            ...
            try {
                ...
                if (Method.POST.equals(method)) {
                    ...
                    files.put("content", saveTmpFile(fbuf, 0, fbuf.limit(), "PUT"));
                }
            } finally {
...
//                                System.out.println("key="+key +", value="+value +"<"); //<<<<<<<<
                                //  Fix space in filename problem
                                if(key.equals("filename")) {
                                    String FN = "filename=\"";
                                    int idx1 = contentDisposition.indexOf(FN);
                                    int idx2 = contentDisposition.indexOf("\"", idx1+FN.length());
                                    value = contentDisposition.substring(idx1+FN.length()-1, idx2+1);  // >"_Name with spaces.txt"<
                                }
...
//                                disposition.put(token.substring(0, p).trim().toLowerCase(), token.substring(p + 1).trim());
                        ...
                        if (item.get("content-type") == null) {
                            ...
                            String path = saveTmpFile(fbuf, offset, bpositions[boundarycount - 1] - offset - 4, value);
...
//                        System.out.println("adding parm key="+pname + " with value="+value); //<<<<<<
                        // adding parm key=theFile with value=10char.txt
         ...
         */
        private String saveTmpFile(ByteBuffer  bBfr, int offset, int len, String fn) {
            String path = "";
            if (len > 0) {
                FileOutputStream fileOutputStream = null;
                try {
                    TempFile tempFile = tempFileManager.createTempFile();
                    String outputFN = tempFile.getName() + "_" + fn;
                    if(!addFnPrefix) {
                        outputFN = savedFilesFolder + fn;
                    }
                    System.out.println("NH sTF fname="+outputFN +", offset="+offset +", len="+len); //<<<<<<<<<<
                    // sTF fname=D:\www\NanoServerTesting\SavedFiles\NanoHTTPD-8854945341675079698, offset=143, len=2980
                    fileOutputStream = new FileOutputStream(outputFN);   //<<<< Writes the data OK with this???
                    FileChannel dest = fileOutputStream.getChannel();

                    ByteBuffer src = bBfr.duplicate();
                    src.position(offset).limit(offset + len);
                    long nbrWrtn = dest.write(src.slice());
//                    System.out.println("sTF wrote nbr="+nbrWrtn); //<<<<<<<<  sTF wrote nbr=10
                    dest.close();            //<<<<<<<<<<<<< ????  still have second file written????

                    path = tempFile.getName();
                } catch (Exception e) { // Catch exception if any
                    System.err.println("NH Error: " + e.getMessage());
                } finally {
        ...
        private RandomAccessFile getTmpBucket() {
            try {
                TempFile tempFile = tempFileManager.createTempFile();     //<<<<<<< WHAT is this used for?
